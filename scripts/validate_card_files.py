...
WHY THE ROW CHECKS LIVE HERE AND NOT IN validate_conformance.py's O4
    ...
    scoreboard is derived from, under `conformance v4`, whose stated bump rule
    ...
    "getting in", O4 is "staying", and widening `conformance v4` to carry an
...
# pattern must match at least one real file across the published tree once
# the tree uses any exemption; a stale pattern matching nothing is caught
# by stale_exemption_breaches.  Patterns are fnmatch-style on the basename.
...
# Reviewed exemption list: each must match something once any exemption is in use.
_EXEMPTION_DIRS: Final[frozenset[str]] = frozenset({"evals"})

# Never reader-facing; never stale-checked. Generated bytecode, VCS, editor junk.
_SKIP_DIRS: Final[frozenset[str]] = frozenset({"__pycache__", ".git"})


# Pre-existing breaches this gate inherits, recorded on the day it landed.
#
# The gate's first run is a MEASUREMENT, not a migration: repairing these cards is
# separate work and belongs with whoever migrates them. Landing the gate red would
# force that migration through the back door, which is what this list prevents.
#
# The list can only SHRINK. An entry whose breach no longer occurs is reported as
# stale and FAILS the gate, so a card that gets fixed forces the entry out rather
# than quietly keeping its exemption. Nothing here can rot unnoticed.
#
# An entry is (card path relative to the repo root, rule kind). Rule kinds are the
# names in _RULES below; a typo is caught because the entry can never match and is
# reported stale on the next run.
...
def _is_exempt(name: str) -> bool:
    ...
    import fnmatch
    if name in _EXEMPTION_DIRS or name in _SKIP_DIRS:
        return True
    if name.startswith("."):
        return True
...
def _extract_local_links(text: str) -> list[str]:
    ...
    for m in re.finditer(r"!?\[[^\]]*\]\(([^)]+)\)", text):
        ...
        if target[:1] != "<":
            ...
            target = target.strip("<>").split()[0] if target else target
...
def _path_exists_case_exact(path: Path) -> bool:
    """True only when *path* exists and every component matches directory entry case.

    ``Path.exists`` alone is not enough on a case-insensitive filesystem: a link
    to ``Bar.md`` would pass when only ``bar.md`` is on disk. Walk each component
    against the real listing so Linux CI and a macOS checkout agree.
    """
    # NOT Path.resolve(): on Windows it returns the real on-disk casing, so a
    # link written 'rEaL.md' comes back as 'Real.md' and the walk below compares
    # the corrected name against itself and always matches. Measured on this host:
    # Path('casetest/rEaL.md').resolve().name == 'Real.md'. os.path.abspath makes
    # the path absolute and normalises separators WITHOUT touching case, which is
    # what a case check needs as input.
    try:
    ...
    # current is the root (e.g. '/'). Walk down matching case at each step.
    cursor = current
...
def link_breaches(card: Path) -> list[str]:
    ...
    for md in sorted(card.rglob("*.md")):
        ...
        for target in _extract_local_links(text):
            joined = md.parent / target
            resolved = joined.resolve()
            # Only require targets that stay inside the card; repo-root links
            # are outside this gate (lychee covers external/HTTP surfaces).
            try:
                resolved.relative_to(card_root)
            except ValueError:
                continue
            # Containment is asked of the RESOLVED path; case is asked of the
            # path as written. Passing `resolved` here is what neutralised this
            # check -- resolve() had already corrected the case it was meant
            # to catch.
            if not _path_exists_case_exact(joined):
...
def reachability_breach(card: Path) -> list[str]:
    ...
    do not count as useful reader links.  Link extraction runs on markdown
    only — following links out of ``.py`` would treat ``arr[0](x)`` as markdown.
    """
    skill = card / "SKILL.md"
    if not skill.is_file():
        return []
    ...
    for md in sorted(card.rglob("*.md")):
        ...
        if _is_exempt(md.name):
            continue
        # Nested paths under an exempt directory (evals/) stay exempt.
        try:
            rel_parts = md.resolve().relative_to(card_root).parts
        except ValueError:
            continue
        if any(part in _EXEMPTION_DIRS for part in rel_parts[:-1]):
            continue
        unreachables.append(md.relative_to(card).as_posix())
...
def stale_exemption_breaches(cards: list[Path]) -> list[str]:
    """Exemption patterns must match at least one file across the published tree.

    Patterns are global, not per-card. Checking them per-card false-positives
    every card that simply has no test fixtures of its own. A tree that uses
    no exemptions at all (isolated single-card fixtures) skips this check —
    there is nothing to go stale against. A tree that uses some exemptions
    must not carry patterns that match nothing.
    """
    import fnmatch
    ...
    def pattern_hits(pat: str) -> bool:
        return any(
            f.is_file() and fnmatch.fnmatch(f.name, pat)
            for card in cards
            for f in card.rglob("*")
        )

    def dir_hits(d_name: str) -> bool:
        return any((card / d_name).is_dir() for card in cards)

    any_exemption_in_use = any(pattern_hits(pat) for pat in _EXEMPTION_PATTERNS) or any(
        dir_hits(d) for d in _EXEMPTION_DIRS
    )
    if not any_exemption_in_use:
        return []

    stale: list[str] = []
    for pat in _EXEMPTION_PATTERNS:
        if not pattern_hits(pat):
            stale.append(pat)
    for d_name in sorted(_EXEMPTION_DIRS):
        if not dir_hits(d_name):
            stale.append(d_name + "/")
    if not stale:
        return []
    return [
        "exemption list names files that no longer exist in any published card: "
        + ", ".join(sorted(stale))
    ]
...
def validate(root: Path) -> None:
    ...
    found: list[tuple[str, str, str]] = []  # (card path, rule kind, detail)
    for card in cards:
    ...
    blocking = [f"  - {rel}: {detail}" for rel, kind, detail in found
                if (rel, kind) not in _ALLOWLIST]

    # The allowlist can only shrink. An entry whose breach no longer occurs has
    # been fixed, and the entry must go with it -- otherwise the list keeps
    # exempting a card that no longer needs it, which is how an allowlist rots
    # into a permanent waiver nobody rereads.
    # An entry naming a card that is not in THIS tree is not applicable, not stale.
    # Without this the fixture trees the suite builds would report all fourteen
    # entries as stale, and the shrink check would fire on trees it says nothing
    # about.
    present = {card.relative_to(root).as_posix() for card in cards}
    live = {(rel, kind) for rel, kind, _ in found}
    applicable = {entry for entry in _ALLOWLIST if entry[0] in present}
    for entry in sorted(applicable - live):
        blocking.append(
            f"  - allowlist entry no longer applies and must be removed: "
            f"{entry[0]} ({entry[1]}). Recorded {_ALLOWLIST_RECORDED}; "
            f"that breach is fixed, so delete the entry."
        )

    # Exemption staleness is tree-wide, never per-card, and is not allowlistable.
    for detail in stale_exemption_breaches(cards):
        blocking.append(f"  - {detail}")

    if allowed:
        print(
            f"ALLOWED: {len(allowed)} pre-existing breach(es) recorded "
            f"{_ALLOWLIST_RECORDED}, not repaired by this gate:",
            file=sys.stderr,
        )
        for rel, kind, detail in allowed:
    ...
    if blocking:
        ...
        print(
            f"REJECTED: {len(blocking)} card contract breach(es) across "
    ...
    print(
        ...
        + (
            f" -- EXCEPT the {len(allowed)} allowlisted breach(es) listed above, "

...
| **Standing cost** | Description ≈ 40 tokens, paid on every turn while the card is model-invocable. The retrieval moment is model-side — an agent writing a CLI test that asserts on an absent environment variable — which is the argument for paying it. Body 6.8 KB, loaded only on retrieval. |

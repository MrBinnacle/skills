...
| **Standing cost** | Description: 56 tokens, measured by `skill-harness skill audit` (skill-harness 0.3.0, 2026-09-22). The retrieval moment is model-side — an agent writing a CLI test that asserts on an absent environment variable — which is the argument for paying it. Body 5,866 B (measured 2026-08-24), loaded only on retrieval. |

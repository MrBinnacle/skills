...
| **Occasions counted** | 1 — 2026-05-27 WCAG-contrast deploy task ([gotchas.md](gotchas.md)); its two symptoms are fan-out from one task, not independent occasions. |
...
| **Standing cost** | Description ≈ 45 tokens if model-invocable. In the author's setup the description is stripped from the model's always-loaded context and the skill is invoked by name when a deploy needs verifying — near-zero standing cost. Installing it normally works too; you just pay the description cost above. Body ≈ 6.7 KB, loads only on invocation. |

# gotchas — halt-as-deliverable (append-only)

- [OBSERVED 2026-06-08] Origin incidents, recorded in `SKILL.md` → Example: the Skill Harness
...
- [OBSERVED 2026-08-16] Second independent occurrence.
  ...
  catalog covering every clause it claims to enforce — and found false. The correction is
  surfaced explicitly in the record, naming both prior wrong passages and the wrongly-cited
  issue number, with the superseded text kept as the record of what was believed; nothing was
  silently fixed. That is this card's "micro-HALT on a stale planning document" variant:
  the catch, not the fix, is the deliverable. Independent of the origin by project, by date
  (ten weeks), and by cause path.

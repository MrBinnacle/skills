...
| Field | Value |
|---|---|
| **Origin** | OBSERVED 2026-05-25, production project ("Writ", SEC-009 push divergence): an agent ran `git pull origin main --no-commit --no-ff` with `pull.rebase=true` configured globally; the rebase silently proceeded, rewriting **22 local commits** and forcing **111 SHA substitutions across 5 state files** plus an authorized force-push recovery. Direct trigger that produced this skill. Full entry: [gotchas.md](gotchas.md) → `[OBSERVED]`. |
...
| **Standing cost** | Description ≈ 90 tokens if model-invocable. In the author's setup it runs as a name-only entry surfaced by an error-signature router rule (near-zero standing cost); adopters can run it either way. Body 3.7 KB. |

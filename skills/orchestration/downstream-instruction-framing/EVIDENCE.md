...
| **Standing cost** | Description ≈ 45 tokens if model-invocable (the author runs it model-invocable — it must fire *before* drafting, which name-only routing can miss). Body ≈ 7 KB, loads only on invocation. |

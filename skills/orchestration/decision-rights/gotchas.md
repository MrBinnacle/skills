...
- **Downstream over-correction.** An agent given full license to disagree can re-open settled
  ...
  what prevents that; keep it when adapting.
...
- **2026-08-23 / the same sweep, run twice, and the first run was void.** The first pass
  ...
  `stale-deploy`. A positive control run afterwards showed that card's
  ...
  `stale-deploy` still has zero occurrences outside the collection, and
  that zero now means something. `closure-mode`: one external hit, an archive
  referencing the card. `pull-rebase`: 34 external hits, every one the card, its
  ...
  **The lesson belongs to the harvest, not to this card:** a sweep that reports absences is a
  ...
  negative-finding run needs. See `vacuous-check` rule 4, which this pass

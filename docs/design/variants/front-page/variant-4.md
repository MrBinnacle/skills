...
>
> **This variant.** Leads with evidence discipline: the `OBSERVED` versus `DESIGNED` provenance
> split, the evidence-state vocabulary, and the per-card evidence table, before naming a single
> card or an install route.

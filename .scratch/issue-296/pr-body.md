...
but the log lives in the private `skills_research` repo. The log path is an input
...
Key behaviours:
- Reads log path from `SKILL_USAGE_LOG` env var; defaults to
  `../skills_research/.claude/state/skill-usage-log.jsonl` relative to repo root.
- **Missing log:** prints `SKIP: usage log not found at <path>`, changes no card, exits 0.
- **Empty log:** rewrites every card's row to zero (`No recorded dispatch, measured <date>`).
...
- Only matches bare skill names, not `plugin:skill` form — those are different install
  paths and adding them would overstate demand.
...
| Control 2: empty log → zero | `TestControl2EmptyLog::test_empty_log_writes_zero` | Empty log rewrites every card to `No recorded dispatch`; nonzero originals change. |
| Control 3: missing log → no change | `TestControl3MissingLog::test_missing_log_no_change` | With `SKILL_USAGE_LOG` unset, every EVIDENCE.md is byte-identical after run. |
| Control 3: skip message | `TestControl3MissingLog::test_skip_message_names_path` | Output contains `SKIP` and `usage log not found`. |

**Observed:** All 11 tests pass. The existing `validate_card_files.py` gate remains green
(14 published cards, all carry SKILL.md/gotchas.md/EVIDENCE.md; every EVIDENCE.md states

...
- **Frontmatter** — `name:` and `description:` are required. `disable-model-invocation:` is permitted under the topology rule below, and nothing else is. Do not add ledger, provenance or resource-inventory fields. Description ≤ 200 chars, written as a *router* ("Use when X, Y, Z") not a summary. Topology (`disable-model-invocation`) is set per the rule below.
...
- **Sizes** — `SKILL.md` 400 bytes to 7,168 bytes, which is the hard ceiling. Target 5,120 bytes or below. Above 5,120, review for duplication, optional material and a coherent extraction, and record the disposition in the review record rather than in the card. Do not split or compress purely to cross the target where that harms correctness, readability or resource discovery. Aux files 400 B to ~3 KB each. [amended 2026-09-06; the prior rule read "If `SKILL.md` is over 5 KB, split." An unconditional split command turns a size guideline into a reason to shave words, and 5,120 and 5,162 bytes have no different operating property. No consumer is known to reject a card at either size; if one is found, that number becomes a compatibility limit and this rule is rewritten around it.]
- **Open by saying what the card does, then show why it is worth having.** A reader who cannot tell what they get will not reach for the card, and a card nobody reaches for costs its description tokens and returns nothing. In the first 150 words of `SKILL.md`: what an agent running this card actually does and what comes back, then one concrete incident drawn from the card's own evidence, with its evidentiary role named. Say which it is — an originating failure that motivated the card, a later application of the discipline, or a catch attributable to this card — and do not present one as another. A date supplies provenance, not causality. Where the record cannot settle the attribution, say so and leave it unsettled. Show, do not assert — "improves code quality" is a claim about the card, "a loose test helper let a list match the string `"1"`" is a reason to install it. Take the example from the card's own evidence and state its limits in the same breath: *"one session, not a measured verdict"* is a stronger pitch than a confident generality, because a reader can check it. A card whose evidence is thin says so here rather than reaching for adjectives.
...
- **Cross-references** — link operating resources where the reader needs them, and say when to open them and what they supply. A resource required to execute the core contract is linked directly at the step that depends on it; moving a required resource behind an unlinked pointer changes the contract, not the layout. Cite evidence beside the claim it supports. Every reader-facing auxiliary is reachable from `SKILL.md` through local links, directly or through another linked document. Test and build resources need not appear in the prose at all. A labelled source list or resource index is permitted where it serves a lookup the inline pointers do not, and never replaces them. [amended 2026-09-06; the prior rule banned a trailing "Related" / "See also" section outright. The ban was obeyed and the positive half was not: measured on the fourteen published cards that day, 21 of 47 reader-facing auxiliaries were named nowhere in their own `SKILL.md`, including `im-up`'s `PACKET-FORMAT.md`, which its card depends on. The count establishes that referencing is incomplete. It does not establish that the ban caused it — that remains a hypothesis, and the fix does not depend on it.]
...
- **`gotchas.md` is required** — append-only log of OBSERVED + ANTICIPATED failure modes. Seed with `[ANTICIPATED]` entries; append an observed entry, or a status update naming the earlier entry, as observations arrive. Never rewrite or delete entries — gotchas are stress-test signal, not failure evidence.
- **These authoring rules are dated, and the frontier moves under them.** Every published card carries a `Re-screen trigger` row because a skill's worth depends on the model reading it. The rules in this section govern those cards and carried no such trigger until now. Rule intent divides on who the rule serves: a rule serving a **human deciding whether to install** (what the card does, what comes back, what evidence exists) is stable as models change; a rule serving an **agent executing** (sentence length, scaffolding, size) is not, because a more capable reader needs less of it. Re-screen this section when a frontier release changes what a card must spell out, and record the date and the finding — including a finding of no change. Do not treat a rule here as settled because it is written down; treat it as measured on a date, against a model, both of which are recoverable from this file. [lodged 2026-09-06]

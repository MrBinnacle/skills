...
- **Sizes** — `SKILL.md` is 400 to 7,168 bytes. Target 5,120 bytes or below. Above 5,120, review for duplication, optional material and a coherent extraction, and record the disposition in the review record. Do not split or compress solely to cross the target where that harms correctness, readability or resource discovery. Aux files 400 B to ~3 KB each. [amended 2026-09-06; rationale in `docs/rule-screens.md`]
...
- **A rule here that makes a claim about model behaviour carries a dated screen, the same as a card carries `EVIDENCE.md`.** This document is not self-authorising: a rule does not become true by being written down here. Authority over the rule set is the maintainer's; authority for a model-behaviour claim inside it is the measurement, and the measurement lives outside this file in `docs/rule-screens.md`. The full mechanism — the five property classes, the trigger, the test surface, the disposition vocabulary and the owner — is specified there. Read it before amending any rule in this section. Two things it settles that matter here: a rule is classified by **the property it preserves**, not by whether a human or an agent reads it, because a human-facing rule can still turn on model behaviour — whether a `description` actually routes retrieval is decided by the model, not the reader; and a screen may return `UNMEASURED`, which is a typed refusal and is recorded as one, never resolved by assertion. [lodged 2026-09-06]
...
The ruled line is `Each card states the condition that would retire it.` — a site may
...
2. **Scan the usage signal.** Report per-card invocation baseline, latest, and delta from
   ...
   **Refresh dispatch rows.** Run `scripts/refresh_dispatch_counts.py` to rewrite every
   ...
   `skills_research` checkout. When the log is absent, no card is changed and the script
...
8. **Reconcile, then validate.** Propagate each count or label change to every derived
   ...
   | `scripts/validate_skill_formats.py` | `scripts/test_validate_skill_formats.py` | a card file whose extension is outside `.md`/`.txt`/`.py`/`.json`, or a `__pycache__` bytecode file with no source beside it. Size bounds live in `validate_card_files.py`, not here |

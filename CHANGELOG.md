...
- [#257](https://github.com/MrBinnacle/skills/pull/257) [`3f4601b`](https://github.com/MrBinnacle/skills/commit/3f4601b488f54b8ef5b941f2022095e9fcdf3299) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - Adds the landing page source under `site/` (one HTML file, one stylesheet, every value drawn from `assets/tokens.json`, seven Phosphor icons vendored inline under MIT, the receipt glyph as the hero mark, the horizontal lockup as the header) and a GitHub Actions Pages workflow that publishes that directory plus the favicon and the social preview and nothing else; `site/index.html` joins the first-person surfaces that `scripts/validate_voice_provenance.py` scans. Adds the favicon set (`assets/favicon.svg`, `site/favicon-32.png`, `site/apple-touch-icon.png`) and the page's title, description and social meta tags. Rebuilds both lockups around the receipt glyph in place of the retired compact mark, exports `assets/social-preview.png` from its SVG source and records the pair's hashes in `assets/tokens.json`, which closes the last two recorded gaps. Every visible line on the page is a candidate for the owner to pick; the social-preview upload to the repository setting is the owner's step.

### Patch Changes

- [#271](https://github.com/MrBinnacle/skills/pull/271) [`c70776f`](https://github.com/MrBinnacle/skills/commit/c70776fab10ebaeabdbbfd703d31ecb87d4d5c61) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - The banner line is retired and replaced. It read _"These aren't the Claude Code skills you're looking for."_ at eleven places across six files; it now reads **"14 skill cards. Each states the condition that would retire it."**, and the sites that lead with the repository name read **"skills. 14 skill cards. Each states the condition that would retire it."**
...
- [#270](https://github.com/MrBinnacle/skills/pull/270) [`fc408d8`](https://github.com/MrBinnacle/skills/commit/fc408d8f1a291e5a29675cdc51ef63d020bbb220) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - `scripts/validate_brand_kit.py` chose the files it scans by walking the filesystem. It asked the working tree what was there, never the index, so the scanned set was whatever a clone happened to hold.
...
- [#260](https://github.com/MrBinnacle/skills/pull/260) [`938faef`](https://github.com/MrBinnacle/skills/commit/938faef2c84d128e6c9c0eb33fdde8986f921dd5) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - Three commit gates now check the surface they were missing: a residue term in a file or directory name is refused in its underscore, hyphen and dot forms; the vale-prose hook lints the same three paths CI lints; and a new `_quarantine/<skill>/` directory is refused unless it carries `LANDING.md` declaring the landing intentional.

- [#258](https://github.com/MrBinnacle/skills/pull/258) [`c9b13fe`](https://github.com/MrBinnacle/skills/commit/c9b13fec696aed5eeceaac8a2b04b3d2eeb70216) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - im-down and im-up: the producer refuses to write a packet after measuring a receiver check red; the card states the computed safe-to-clear verdict; a REJECTED receipt says whether every packet assertion held and names the failing receiver checks.

- [#264](https://github.com/MrBinnacle/skills/pull/264) [`449eab5`](https://github.com/MrBinnacle/skills/commit/449eab5b6f4f90d51bfdc5b12c369ad43e357de2) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - One word list now binds every line of prose this repository publishes. `assets/tokens.json > copy.words_to_avoid_surfaces` gains a `markdown_prose` surface over every tracked `.md` except `_quarantine/**`, whose candidates are frozen, and `CHANGELOG.md`, whose entries record what shipped under the wording in force at the time. `scripts/validate_brand_kit.py` reads that surface, and the fixture that used to assert body prose passed with a listed word is inverted into a poison control that goes red when it does. Twenty-five lines were rewritten, each to name the thing the sentence was pointing at rather than the word standing in for it.
...
- [#269](https://github.com/MrBinnacle/skills/pull/269) [`a862f0f`](https://github.com/MrBinnacle/skills/commit/a862f0fc6c05168b62fc7266435089d3da52d841) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - `README.md` is rewritten. The copy and the composition are new; the facts, the tables, the counts and every CI-locked string are carried across unchanged.
...
- [#273](https://github.com/MrBinnacle/skills/pull/273) [`9496a5c`](https://github.com/MrBinnacle/skills/commit/9496a5c1f7ed2ac5ed2fe00a87ddaa948ec1a58f) Thanks [@MrBinnacle](https://github.com/MrBinnacle)! - `README.md` now states what an install actually writes. Three claims on the page were false, and a cold-install run measured each one.

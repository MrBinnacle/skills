...
Each ships its own poison control under the release-gate job in CI, asserting its own distinguishing message and a single-reason refusal. The release-gate job now sets up node (pinned) so the G6 control can run `npx`; that line is itself a `uses:` G7 re-asserts is pinned. The checks skip cleanly when there is nothing published to re-assert, so the seeded-tree lockstep/changeset/changelog cases stay single-reason.

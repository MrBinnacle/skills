...
- `router-skill-predicate-gap` 8736 → 6975 B. The origin incident was told three times
...
- `success-test-accepts-any-output` 8197 → 6830 B. The 2026-08-17 outage incident appeared
...
- `halt-as-deliverable` 7422 → 5423 B. Steps 2 and 4 ("resist the quiet re-run" / "make it
  ...
  aside folded into that step; two Notes that restated Solution steps are cut. The decision
  table and the four-point verification list survive intact.
- `click-clirunner-env-none-deletes` 6582 → 5780 B. The monkeypatch advice appeared in both
...
`mock-masked-stub-trap` (4156 B), were assessed under the same pass and left unchanged:
both are under every threshold and carry no internal duplication worth a diff.

Each edited card's `EVIDENCE.md` standing-cost row is re-measured to the new body size.
All seven repository validators and the spec validator (`skills-ref` 0.1.5) pass over the
edited tree. An independent fresh-context review of the cuts found four defects, all fixed
here: the compression had deleted a Notes sentence that `gotchas.md` quotes verbatim (the
"a router rule deserves a test suite" claim — restored); a `§ 1b` pointer in that same
...
above); and the draft described that review in the past tense before it had run — the
